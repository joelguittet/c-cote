...
static void display_nodes(cote_t *cote) {
  ...
  /* Print title */
  char *name = format_string("Name", 20); char *iid = format_string("Instance ID", 40);
  char *address = format_string("Address", 18); char *port = format_string("Port", 5);
  printf("\033[32m%s %s %s %s\033[0m\n", name, iid, address, port);
  free(name); name = NULL; free(iid); iid = NULL; free(address); address = NULL; free(port); port = NULL;
  ...
  while (NULL != tmp) {
    ...
    cJSON *port_json = NULL; int port_number = 0;
    if (NULL != tmp->data.advertisement) {
